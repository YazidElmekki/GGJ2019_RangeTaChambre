Shader "Custom/PostRendering/Pencil (Daniel Taylor version)"
{
	...
	Properties
	{
		...
		_DirMultiplicator("Dir multiplicator", float) = 1.0
		_BlendColor("Blend Color", Color) = (0,0,0,0)
		_Sensitivity("Sensitivity", float) = 10.0
		...
		SubShader
		{
			...
			Pass
			{
				...
				float2x2 rotMatrix()
				{
					float sinX = sin(_RotationAngle);
					float cosX = cos(_RotationAngle);
					float sinY = sin(_RotationAngle);
				...
				void pR(inout float2 p, float a) {
					p = cos(a) * p + sin(a) * float2(p.y, -p.x) + _DirMultiplicator;
				}
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					for (int j = 0; j < ANGLENUM; j++)
					{
						...
						for (int i = -RANGE; i <= RANGE; i += STEP)
						{
							...
							float2 pos2 = screenPos + float2(b.x, b.y) * i + float2(_Time.y * _TimeScale, _Time.y * _TimeScale);
							//pos2 += _ScreenParams.xy * 0.5;
							pos2 = mul(pos2, rotMatrix());
							pos2 = abs(pos2);
							pos2 = fmod(pos2, _ScreenParams.xy);
							...
							weight -= pow(abs(dot(normalize(grad), normalize(g))), _Sensitivity) / floor((2.0 * RANGE + 1.0) / STEP) / ANGLENUM;
						}
					}
					...
					return lerp(_BlendColor, background, weight);
				}
